...
#===========================================================
```{r}
#Correlation test between price and mileage
price_mileage_cor <- new_car[, c('price', 'mileage')]

cor(price_mileage_cor)
library(corrplot)

corrplot(cor(price_mileage_cor), method='number', is.corr = FALSE)

#test test for price and mileage
t.test(new_car$price, new_car$mileage)
...
#Stepwise selection
multilinear <- lm(price ~., data=new_car)
multilinear

library(MASS)
stepAIC(multilinear, direction='both', trace = FALSE)
...
ggplot(new_car2, aes(color_collapsed)) +
  geom_bar() +
  theme(axis.text.x = element_text(angle = 90))
```
...
```{r}
ggplot(new_car_collapsed_knn, aes(color_collapsed)) +
  geom_bar() +
  theme(axis.text.x = element_text(angle = 90))
...
most_expensive[,c('brand', 'price', 'model')]
```
...
price_outcome <- KNN_reg[,'price'] #%>% select(price)
KNN_reg <- KNN_reg[,-1] #%>% select(-price)
